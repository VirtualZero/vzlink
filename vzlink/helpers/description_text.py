def get_current_timestamp_description_text():
    ...
    <a href="#">https://datetimeapi.virtualzero.tech/api/datetime/get-current-timestamp?tz=America/New_York&tz=America/Denver</a>. 
    ...
    <a href="#">https://datetimeapi.virtualzero.tech/api/datetime/get-current-timestamp?tz=America/New_York&elements=False</a>. 
    ...
    <a href="#">https://datetimeapi.virtualzero.tech/api/datetime/get-current-timestamp?tz=America/New_York&only_elements=True</a>. 
...
def api_description_text():
    ...
    cloned on <a href='https://github.com/VirtualZero/datetime-utils-api' target='_blank'>\
    GitHub</a>. Additionally, a preconfigured virtual machine in OVA format \
    is available for download <a href='https://datetimeapi.virtualzero.tech/#download' target='_blank'>here</a>."""
...
def convert_timezones_description_text():
    ...
    (without the elements), simply use elements=False as a query 
    string parameter. Example: 
    <a href="#">https://datetimeapi.virtualzero.tech/api/datetime/convert-timezones?elements=False</a>. 
    To exclude timestamps and retrieve ONLY the elements that the 
    timestamp is comprised of, simply use only_elements=True as a query 
    string parameter. Example: 
    <a href="#">https://datetimeapi.virtualzero.tech/api/datetime/convert-timezones?only_elements=True</a>. 
    For obvious reasons, if both only_elements=True and elements=False 
    are used as query string parameters, the API will return a 
    successful response, but the response will contain no data.</p>"""
...
def addition_description_text():
    ...
    to 0 (zero). Example: 
    <a href="#">https://datetimeapi.virtualzero.tech/api/datetime/addition?timestamp_now=True</a> 
    The default action of the API is to return all of the individual 
    elements that the timestamp is comprised of, in addition to the 
    actual timestamps. To retrieve ONLY the actual timestamps 
    (without the elements), simply use elements=False as a query 
    string parameter. Example: 
    <a href="#">https://datetimeapi.virtualzero.tech/api/datetime/addition?elements=False</a>. 
    To exclude timestamps and retrieve ONLY the elements that the 
    timestamp is comprised of, simply use only_elements=True as a 
    query string parameter. Example: 
    <a href="#">https://datetimeapi.virtualzero.tech/api/datetime/addition?only_elements=True</a>. 
    For obvious reasons, if both only_elements=True and elements=False 
    are used as query string parameters, the API will return a 
    successful response, but the response will contain no data.</p>"""
